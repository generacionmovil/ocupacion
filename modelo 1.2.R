...
#library(sqldf)
#dat = dat[dat$class == 'Test',]
#dat <- dat %>% select(-class)
...
library(sqldf)
train <- sqldf("select * from dat limit 68737")
...
#Regresion lineal
modelo1 <- lm(data = traintt, formula = Ocupacion ~ Valorlluvia + year.2019 + year.2020 + yday 
...
write.table(forecast(modelo1, newdata = testt, h=25600),"C:/Users/Daniel/OneDrive - Pontificia Universidad Javeriana/Attachments/Maestria/Tesis/Proceso/Mserie/Full Hogar/Fullhogarforecast2.1.csv", sep = ";", dec = ",")
